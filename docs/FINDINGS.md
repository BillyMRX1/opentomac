...
## F4 status update (2026-07-18)

Debugged end to end with adb-posted test notifications: phone listener connects, posts reach the wire (`notification -> Mac: NotificationPosted state=Connected`), the Mac EVENT handler receives them, and `UNUserNotificationCenter.add` accepts the banner without error ("banner accepted" logged). macOS notification permission is granted. If no banner is visible, the suppression is at the macOS display layer: Focus/Do Not Disturb, or alert style set to None; Notification Center history should still list accepted banners. Fixed along the way: ongoing notifications (charging state, every 10 s) and group summaries (WhatsApp double-send) are no longer mirrored; the Android "Enable notification mirroring" button hides once access is granted (by design). Also fixed in this round: quick-settings tile now opens the app (tiles cannot read the clipboard on Android 10+), text-selection action boots the runtime with a bounded wait, and the phone auto-connects on startup. Watch out for multiple concurrent Mac app instances during testing: a stale instance holds port 42420 and receives everything while the fresh one gets nothing.
