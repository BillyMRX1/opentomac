...
1. DONE 2026-07-21 (commit 9d32a09): Mac sidebar navigation. `RootView.swift` hosts a `NavigationSplitView` (sidebar: Dashboard, Photos, Contacts, Messages, Calls); the four browse views are detail pages (isPresented bindings and Close buttons removed, all lifecycle/PII behavior kept), the `mirrorPresentationTick` onChange and notification-permission refresh live on the split-view root, Mirror stays its own Window, pairing stays a dashboard sheet, main window min is 1000x640. Built via Sonnet subagent from a Fable plan, re-verified with `scripts/build-macos.sh`, relaunched via `open` with the 42420 listener confirmed. Not yet visually inspected by the user.
2. DONE 2026-07-21: back-button + mirror-fix APK installed on R5GL60KRH3N (`adb install -r`, Success, no signature mismatch, no re-pair needed).
2b. DONE 2026-07-21 (commit 16625d5): Wi-Fi-gated reconnect + calm notification. The user was spammed by "Connecting, attempt N" whenever away from the Mac. New `NetworkMonitor` (Wi-Fi/Ethernet callback); AppRuntime stops the reconnect loop on Wi-Fi loss, reconnects instantly on return, gates startup auto-connect, and errors a manual connect off-Wi-Fi. Notification text is now static ("Looking for your Mac…" / "Waiting for Wi-Fi", no counter) and notify() is skipped when unchanged; in-app status matches. shared/ untouched. Device-verified end-to-end via `adb shell svc wifi disable/enable` (note: after Wi-Fi re-enable the Samsung takes a while to rejoin and the first attempts fail while the datapath settles, so reaching Connected can take a couple of minutes — that's backoff, not a bug). APK installed on the phone.
