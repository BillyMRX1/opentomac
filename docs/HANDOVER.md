...
4. Phase E: SMS/calls (gate on Play policy review). Phase F: iPad + nearby drop. Phase D webcam/mic was dropped and removed.

## Task state

All 13 implementation-plan tasks completed. Roadmap Phases A and B completed. Phase B device-verified by the user on 2026-07-18 (screenshot flow reworked to offer-first and the Mac's redundant open-link button removed on their feedback). Phases C and E were implemented 2026-07-18 (see the roadmap doc for design and accepted limits), pending user device test. C: screen mirroring + viewer window + remote control. E: SMS browse/reply + call log (send guarded by an on-phone confirmation). Phase D phone-as-webcam/mic was dropped by the user and removed completely; do not reintroduce its protocol, capture services, permissions, dependencies, or Mac preview UI. Screen mirroring still owns the VIDEO channel and its `VideoConfig`/`VideoFrame` messages. Remaining roadmap: Phase F (iPad companion, nearby drop) and the small polish backlog (Mac Keychain, single-instance guard, higher-res photo previews, automated two-device test). Device-verified so far: clipboard all types with auto-sync, file transfer + cancel, photos grid + preview + import, notification mirroring + banner reply, screenshot offers, URL handoff phone-to-Mac, media remote, contacts search, quick-settings tile, auto-connect. Still unverified: the fixed text-selection action. Both apps rebuilt and verified after every change.
